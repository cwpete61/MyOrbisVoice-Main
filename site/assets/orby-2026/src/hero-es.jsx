      ...
      <div className="container hero-inner">
        <div className="hero-copy">
          <div className="hero-eyebrow">
            ...
            <span className="eyebrow">Orby · agente IA · en vivo en 60 segundos</span>
          </div>

          <h1 className="h-display hero-h1">
            No nos creas a nosotros <br />
            <span className="hero-h1-accent">lo que Orby puede hacer…</span>
          </h1>

          <p className="hero-sub">
            Una sola recepcionista IA que <strong>contesta cada llamada</strong>, <strong>agenda en tu calendario</strong>,
            y <strong>da seguimiento por SMS, correo y WhatsApp</strong> — mientras tú estás metido hasta el codo en el trabajo real.
            Reemplaza 4–6 herramientas separadas. Habla inglés y español. Nunca duerme.
          </p>

          <div className="hero-cta-row">
            <a href="#orby-demo" className="btn btn-cta btn-cta-big">
              <span className="mic-glyph">🎙</span> ¡Habla con Orby AHORA!
            </a>
            <a href="#how" className="btn btn-ghost">Mira cómo funciona ↓</a>
          </div>

          <div className="hero-trust">
            <div className="trust-stat">
              <div className="trust-n">40%</div>
              <div className="trust-l">de las llamadas entrantes<br/>quedan sin contestar en la industria</div>
            </div>
            <div className="trust-divider" />
            <div className="trust-stat">
              <div className="trust-n">$1,491</div>
              <div className="trust-l">costo mensual de las<br/>4–6 herramientas que reemplazamos</div>
            </div>
            <div className="trust-divider" />
            <div className="trust-stat">
              <div className="trust-n">7</div>
              <div className="trust-l">agentes especialistas,<br/>una sola voz, cero transferencias</div>
            </div>
          </div>
        </div>
        ...
        <div className="hero-stage" id="orby-demo">
          ...
          <FloatingCard className="float-1" delay="0s">
            ...
            <div className="float-card-body">
              <div className="float-card-name">Maria S. — Dolor de muela</div>
              <div className="float-card-meta mono">Hoy · 2:15 PM</div>
            </div>
          </FloatingCard>
          <FloatingCard className="float-2" delay="1.2s">
            <div className="float-card-tag" style={{background: "#FF7C7C"}}>RECOVERED</div>
            <div className="float-card-body">
              <div className="float-card-name">James K. → saliente en 4 min</div>
              <div className="float-card-meta mono">SMS + respaldo por voz</div>
            </div>
          </FloatingCard>
          <FloatingCard className="float-3" delay="2.4s">
            <div className="float-card-tag" style={{background: "#7C9CFF"}}>SMS DISPATCHED</div>
            <div className="float-card-body">
              <div className="float-card-name">17 recordatorios · 1h antes</div>
              <div className="float-card-meta mono">Agenda de mañana</div>
            </div>
          </FloatingCard>
        </div>
      </div>
  ...
  return (
    <div className="hero-split">
      <div className="hs-side hs-before">
        <div className="hs-label mono">Antes de Orby</div>
        <div className={`phone-card ${phase === 0 ? "ringing" : "missed"}`}>
          <div className="phone-screen">
            <div className="phone-time">11:47 PM</div>
            <div className="phone-status">{phase === 0 ? "Llamada entrante…" : "Llamada perdida · 3ª esta semana"}</div>
            <div className="phone-name">+1 (610) 555‑0144</div>
            <div className="phone-glyph">{phase === 0 ? "📞" : "📵"}</div>
            <div className="phone-vm">Buzón de voz no configurado</div>
          </div>
        </div>
        <div className="hs-loss">
          <span className="hs-loss-n">$340</span>
          <span className="hs-loss-l">pérdida promedio por llamada perdida</span>
        </div>
      </div>
      ...
      <div className="hs-side hs-after">
        <div className="hs-label mono" style={{color: "var(--accent-hi)"}}>Con Orby</div>
        <div className="orby-card">
          <div className="orby-card-head">
            ...
            <div>
              ...
              <div className="mono" style={{fontSize: 10, color: "var(--text-3)", letterSpacing: "0.12em", textTransform: "uppercase"}}>{phase >= 1 ? "Activo · Agente de Citas" : "En espera"}</div>
            </div>
          </div>
          <div className="orby-card-body">
            {phase === 0 && <Line>Timbre detectado — contestando al segundo timbre…</Line>}
            {phase >= 1 && (
              <>
                <Line who="orby">"Allentown Family Dental, habla Orby."</Line>
                <Line who="caller">"Me está matando la muela, ¿puedes meterme hoy?"</Line>
                <Line who="orby">"Hoy a las 2:15. Agendado. Confirmación enviada."</Line>
              </>
            )}
          </div>
          {phase >= 2 && (
...
function Line({ who, children }) {
  return (
    <div className={`hs-line hs-line-${who || "sys"}`}>
      {who && <span className="hs-line-tag">{who === "orby" ? "Orby" : "Cliente"}</span>}
      {children}
      <style>{`
...
function LogoStrip() {
  // "Integrated with" — generic placeholder logos
  const items = ["Google Calendar", "Gmail", "Stripe Connect", "Twilio", "Gemini Live", "OpenAI", "WhatsApp", "WordPress", "Bunny CDN", "Reoon"];
  return (
    <div className="container" style={{marginTop: 80}}>
      <div className="logo-strip-label mono">
        <span className="eyebrow">Integraciones en vivo · haciendo trabajo real hoy</span>
      </div>
      ...
      <style>{`
        ...
        .logo-glyph {
          ...
          border-radius: 6px;
          background:
            linear-gradient(135deg, color-mix(in oklab, var(--brand) 40%, transparent), color-mix(in oklab, var(--brand-hi) 40%, transparent));
