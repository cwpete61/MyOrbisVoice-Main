...
const ORBY_SCRIPT = [
  { who: "orby", role: "Secretary", text: "Allentown Family Dental, this is Orby. How can I help?" },
  { who: "caller", text: "Hi, my tooth's been killing me all weekend. Can I get in today?" },
  { who: "orby", role: "Appointment", text: "I'm sorry you're in pain. Let me check Dr. Patel's calendar for today. I see an opening at 2:15 PM. Will that work?" },
  { who: "caller", text: "Yeah, 2:15 works. It's Maria Sanchez." },
  { who: "orby", role: "Appointment", text: "Got it, Maria. I see you were here in February for a cleaning. Same insurance, Aetna?" },
  { who: "caller", text: "Yes, same one." },
  { who: "orby", role: "Appointment", text: "Booked. I just texted and emailed the confirmation. We'll see you at 2:15. Feel better, Maria." }
];
...
function OrbyDemo() {
  ...
  const [currentRole, setCurrentRole] = useState("Secretary");
      ...
      <div className="orby-demo-head">
        <div className="orby-meta">
          ...
          <span className="mono" style={{fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--text-2)"}}>
            { state === "idle" ? "Orby standing by"
            : state === "ringing" ? "Inbound call · ringing"
            : state === "active" ? `Live · ${fmtTime(duration)}`
            : `Call ended · ${fmtTime(duration)}` }
          </span>
        </div>
      ...
      <div className="orby-stage">
        ...
        {(state === "active" || state === "done") && (
          <div className="orby-role-pill" style={{['--pill']: ROLE_COLORS[currentRole]}}>
            <span className="dot" />
            <span className="mono">{currentRole} agent</span>
          </div>
        )}
      </div>
        ...
        {state === "idle" && (
          <div className="orby-empty">
            ...
            <p style={{color: "var(--text-2)", margin: "8px 0 0", fontSize: 14}}>
              Tap the button below. A real Orby conversation will play out — same engine your customers will hear.
            </p>
          </div>
        )}
        {turns.map((t, i) => (
          <div key={i} className={`turn turn-${t.who}`}>
            <div className="turn-label mono">{t.who === "orby" ? `Orby · ${t.role}` : "Caller"}</div>
        ...
        {state === "done" && (
          <div className="outcome-card fade-in">
            <div className="outcome-row">
              ...
              <div className="mono outcome-time">{fmtTime(duration)} · 7 turns</div>
            </div>
            <div className="outcome-grid">
              <div>
                <div className="outcome-k">Customer</div>
                <div className="outcome-v">Maria Sanchez (returning)</div>
              </div>
              <div>
                <div className="outcome-k">Appointment</div>
                <div className="outcome-v">Today · 2:15 PM · Dr. Patel</div>
              </div>
              <div>
                <div className="outcome-k">Auto-dispatched</div>
                <div className="outcome-v">SMS confirmation · Email confirmation · 1h reminder armed</div>
              </div>
              <div>
                <div className="outcome-k">CRM updated</div>
                <div className="outcome-v">Visit history · Insurance: Aetna</div>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="orby-controls">
        ...
        {(state === "ringing" || state === "active") && (
          <button className="btn btn-ghost" onClick={reset}>
            <span style={{color: "#ff7c7c"}}>●</span> End call
          </button>
        )}
        {state === "done" && (
          <div style={{display: "flex", gap: 12, flexWrap: "wrap", justifyContent: "center"}}>
            ...
            <button className="btn btn-ghost" onClick={reset}>Try another scenario →</button>
          </div>
        )}
      </div>
