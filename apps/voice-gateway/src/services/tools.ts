...
export function buildToolGuidanceBlock(): string {
  return [
    ...
    '- ECHO BACK BEFORE SAVING. Once you have name + phone + email, READ ALL THREE BACK to the caller verbatim and ask them to confirm. Use this template: "Let me make sure I have this right — your name is [name], phone number [phone, said clearly digit-by-digit], and email [email, spelled out: a-l-e-x at example dot com]. Did I get all that correctly?" If they correct anything, repeat the corrected piece back to them, then call save_contact. Never call save_contact without explicit verbal confirmation.',
    '- Call save_contact as soon as the caller has confirmed the echo-back. Don\'t wait until the end of the call.',
    ...
    '',
    'Rules — keep the line warm (no dead air):',
    '- BEFORE calling any tool, narrate briefly what you\'re doing. Examples: "Let me check that for you", "One moment while I look that up", "Just pulling up the calendar now", "Got it — let me save that to your record", "Sending that email right now".',
    '- AFTER a tool returns, share the result naturally. Don\'t go silent waiting for the next thing.',
    '- If you genuinely need a beat to think mid-conversation, say something brief like "Just a second" or "Let me think about that for a moment" — never just stop talking. Callers should never have to ask "are you still there?"',
    '',
    'Rules — general:',
    '- Confirm details verbally before any write tool (book_appointment, send_followup_email). Spell back email addresses character by character.',
