...
export function buildToolGuidanceBlock(): string {
  return [
    ...
    '- When the caller indicates this is a tech support / help / "something is broken" call, gather diagnostic info BEFORE booking. The support rep taking the appointment needs to walk in informed — every minute of context you capture saves them five minutes of discovery on the actual call.',
    '- Ask the questions ONE AT A TIME, wait for each answer. Three required questions, in order:',
    ...
    '- Stop after 3 questions if the picture is clear. Don\'t make a stressed/frustrated caller answer 5 diagnostic questions when 3 are enough.',
    '- After intake, proceed normally: contact capture (name → phone confirm → email), then book.',
    '- When calling book_appointment, populate the `notes` field with a CONCISE summary of what the caller told you — 1-3 sentences max. Capture: the issue, when it started, urgency, and any specifics. Example: "Caller can\'t log into the dashboard, started this morning. Already tried clearing cookies. Blocked from work — urgent."',
    '- This intake flow is for tech support ONLY. For demos: skip all of this. Demos are exploratory — just collect contact info and book. Don\'t interrogate a demo caller.',
    '',
