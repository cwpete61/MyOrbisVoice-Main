    ...
    articles: [
      {
        ...
        steps: [
          ...
          { title: 'Sidebar layout', body: 'Platform section: Overview, System Settings, Platform Status. Management: Tenants, Plans, Partners, Twilio Call Logs. Account: Profile, Help.', screenshots: [{ filename: 'admin-sidebar-full.png', caption: 'Full admin left sidebar showing all three sections (Platform / Management / Account) with every link visible', capture: { url: '/admin', authAs: 'admin' } }] },
    ...
    articles: [
      {
        ...
        steps: [
          { title: 'Open the Tenants list', body: 'Click "Tenants" in the Management section of the admin sidebar. The list shows every tenant in the system with status badges (TRIAL / ACTIVE / SUSPENDED / PAST_DUE).', screenshots: [{ filename: 'admin-tenants-list.png', caption: 'Admin Tenants page showing the searchable tenant list with status badges', capture: { url: '/admin/tenants', authAs: 'admin' } }] },
          { title: 'Search by name or email', body: 'Use the search bar at the top to filter by display name or registration email. Matches happen as you type.', screenshots: [{ filename: 'admin-tenants-search-bar.png', caption: 'Top of Tenants list with the search input and active query highlighting matching rows', capture: { url: '/admin/tenants', authAs: 'admin' } }] },
          { title: 'Click a tenant to open detail', body: 'Click any row to land on the tenant detail page. From there you can see all members, integrations, subscription state, conversation/appointment/contact counts, storage usage, and take admin actions.', screenshots: [{ filename: 'admin-tenant-detail-overview.png', caption: 'Full admin tenant detail page showing the Details / Plan-Tier / Activity / Members / Integrations / Recording Storage cards', capture: { url: '/admin/tenants', authAs: 'admin', setup: [{ action: 'click', selector: 'a[href^="/admin/tenants/"]' }, { action: 'wait', ms: 1500 }], fullPage: true } }] },
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'How to suspend', body: 'On the tenant detail page, click the red "Suspend" button at the top right. Confirm in the dialog.', screenshots: [{ filename: 'admin-tenant-suspend-button.png', caption: 'Top-right of tenant detail page with the red Suspend button', capture: { url: '/admin/tenants', authAs: 'admin', setup: [{ action: 'click', selector: 'a[href^="/admin/tenants/"]' }, { action: 'wait', ms: 1500 }] } }] },
          ...
          // admin-tenant-restore-button.png — captured manually: requires a tenant in SUSPENDED state. Recreate by suspending a non-critical test tenant in the admin UI (light mode), screenshotting the top-right Restore button, then restoring the tenant. Not auto-captured because the script lacks a post-capture cleanup hook and we don't want to leave a tenant permanently suspended for screenshots.
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'Click "Enter as tenant"', body: 'On the tenant detail page, click the "Enter as tenant →" button at the top right. This issues a short-TTL impersonation token and redirects you into their dashboard.', screenshots: [{ filename: 'admin-enter-as-tenant-button.png', caption: 'Top-right of tenant detail page showing the Enter as tenant button next to Suspend', capture: { url: '/admin/tenants', authAs: 'admin', setup: [{ action: 'click', selector: 'a[href^="/admin/tenants/"]' }, { action: 'wait', ms: 1500 }] } }] },
          // admin-impersonation-banner.png — capture single-shot ONLY (not in batch). The
          // impersonation flow swaps the admin session for an impersonation token, which
          // pollutes the shared browser context for every subsequent admin capture in
          // the same run. Recapture with:
          //   pnpm capture-screenshots --filename admin-impersonation-banner.png
          // Setup: open /admin/tenants → click any tenant link → wait 1500 → click
          // [data-testid="impersonate-btn"] → wait 2500 → screenshot top of /dashboard.
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'Find the panel', body: 'On the tenant detail page, look for the card titled "Plan / Tier (Admin Grant — Bypasses Stripe)". It has a plan dropdown and a Grant button.', screenshots: [{ filename: 'admin-grant-plan-card.png', caption: 'Tenant detail page showing the Plan/Tier admin grant card with the dropdown and Grant button', capture: { url: '/admin/tenants', authAs: 'admin', setup: [{ action: 'click', selector: 'a[href^="/admin/tenants/"]' }, { action: 'wait', ms: 1500 }], fullPage: true } }] },
    ...
    articles: [
      {
        ...
        steps: [
          { title: 'Open the Plans page', body: 'Click "Plans" in the Management section of the admin sidebar. The page lists all 6 plan tiers as collapsible accordions.', screenshots: [{ filename: 'admin-plans-list.png', caption: 'Admin Plans page showing all 6 tier accordions in collapsed state', capture: { url: '/admin/plans', authAs: 'admin' } }] },
          { title: 'Expand a plan', body: 'Click any plan card to expand its entitlements. You will see all 35+ entitlement keys (max_channels, max_agents, widget_enabled, etc.) with editable input fields next to each.', screenshots: [{ filename: 'admin-plan-expanded.png', caption: 'A plan accordion expanded showing the full list of entitlements with edit fields', capture: { url: '/admin/plans', authAs: 'admin', setup: [{ action: 'click', selector: '[data-testid="plan-accordion-toggle"]' }, { action: 'wait', ms: 500 }], fullPage: true } }] },
    ...
    articles: [
      {
        ...
        steps: [
          ...
          { title: 'Open System Settings', body: 'Click "System Settings" in the Platform section of the admin sidebar. You will see one card per integration (Stripe, Twilio, Google, OpenAI, Bunny, Reoon, SMTP).', screenshots: [{ filename: 'admin-system-settings-cards.png', caption: 'Admin System Settings page showing all 7 integration cards with status indicators', capture: { url: '/admin/system-settings', authAs: 'admin', fullPage: true } }] },
          { title: 'Update a key', body: 'Click the integration card you need. Each has write-only inputs (the existing value is shown as masked dots, never the actual value). Paste the new value, click Save. The value is encrypted before being stored.', screenshots: [{ filename: 'admin-system-settings-stripe-card.png', caption: 'Stripe card expanded showing Secret Key / Publishable Key / Webhook Secret input fields with Save button', capture: { url: '/admin/system-settings', authAs: 'admin', fullPage: true } }] },
