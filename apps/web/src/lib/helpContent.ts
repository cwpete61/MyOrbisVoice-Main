    ...
    articles: [
      {
        ...
        summary: 'Understand how MyOrbisVoice works and what each part of the platform does.',
        steps: [
          { title: 'What is MyOrbisVoice?', body: 'MyOrbisVoice is a multi-channel AI voice automation platform. It puts an intelligent voice agent on your website widget, your phone line, and your outbound calling campaigns — all configured from one dashboard.' },
      ...
      },
      {
        ...
        steps: [
          { title: 'Step 1 — Fill in Business DNA', body: 'Go to Business DNA in the sidebar. Complete at minimum: your business name, industry, primary service, operating hours, and one escalation condition. Publish the draft when ready.', screenshots: [{ filename: 'nav-business-dna.png', caption: 'Sidebar \'Business DNA\' link highlighted, in the Build Your Agent section', capture: { url: '/dna', authAs: 'tenant' } }] },
          { title: 'Step 2 — Write your Master Prompt', body: 'Go to Prompts. Create a new prompt with scope "Master". Describe your agent\'s persona, tone, and primary goal in plain language. Publish it.', screenshots: [{ filename: 'nav-prompts.png', caption: 'Sidebar \'Prompts\' link highlighted, in the Build Your Agent section', capture: { url: '/prompts', authAs: 'tenant' } }] },
          ...
          { title: 'Step 4 — Test in Agent Studio', body: 'Go to Agent Studio. Select the Widget tab. Pick a voice. Click "Start Test" and speak to your agent. Check that responses match your Business DNA.', screenshots: [{ filename: 'nav-agent-studio.png', caption: 'Sidebar \'Agent Studio\' link, plus the Agent Studio page with the \'Start Test\' button visible', capture: { url: '/agent-studio', authAs: 'tenant' } }] },
          { title: 'Step 5 — Embed the widget', body: 'Go to Channels → Widget. Copy the embed snippet and paste it into the <head> of your website.', screenshots: [{ filename: 'channels-widget-embed-snippet.png', caption: 'Channels page with the Widget card expanded and the \'Copy Embed Code\' button visible', capture: { url: '/channels', authAs: 'tenant' } }] },
        ],
    ...
    articles: [
      {
        ...
        steps: [
          ...
          { title: 'Versioning', body: 'MyOrbisVoice keeps a full history of your Business DNA drafts. You can have one active version at a time. Old versions are preserved so you can review or revert.' },
        ],
      },
      {
        id: 'dna-editing',
        ...
        steps: [
          ...
          { title: 'Save the draft', body: 'Click "Save Draft" at any time. The draft is not live — it\'s saved to your account but the active agent still uses the previously published version.', screenshots: [{ filename: 'dna-save-draft-button.png', caption: 'Bottom-right of the DNA editor showing the \'Save Draft\' button', capture: { url: '/dna', authAs: 'tenant' } }] },
          { title: 'Publish', body: 'When you\'re satisfied with your draft, click "Publish". A confirmation dialog will appear. Once confirmed, this version becomes the active Business DNA immediately — all new conversations use it.', screenshots: [{ filename: 'dna-publish-button.png', caption: 'Bottom-right of the DNA editor showing the teal \'Publish\' button next to \'Save Draft\'', capture: { url: '/dna', authAs: 'tenant' } }, { filename: 'dna-publish-confirm-dialog.png', caption: 'Modal dialog asking \'Publish this version of Business DNA?\' with confirm/cancel buttons', capture: { url: '/dna', authAs: 'tenant' } }] },
        ],
        tips: [
    ...
    articles: [
      {
        ...
        summary: 'MyOrbisVoice uses a layered prompt system. Learn what each layer does and when to use each one.',
        steps: [
          ...
          { title: 'Layer 1 — Platform prompt (hidden)', body: 'This is set by MyOrbisVoice and cannot be edited. It handles safety, format requirements, and baseline behavior. You never see or edit this layer.' },
      ...
      },
      {
        ...
        steps: [
          { title: 'Creating a new prompt', body: 'Click the "+ New Prompt" button. Select a scope (Master, Widget, Inbound, Outbound, or Role). Give it a name you\'ll recognize (e.g. "Master — Booking Focus v2"). Write the content and save as a draft.', screenshots: [{ filename: 'prompts-new-button.png', caption: 'Prompts page top-right with the \'+ New Prompt\' button', capture: { url: '/prompts', authAs: 'tenant' } }, { filename: 'prompts-scope-dropdown.png', caption: 'Prompt creation modal with the Scope dropdown open showing Master/Widget/Inbound/Outbound/Role options', capture: { url: '/prompts', authAs: 'tenant' } }] },
  ...
  },
  {
    ...
    articles: [
      ...
      },
      {
        ...
        steps: [
          { title: 'Open an agent role', body: 'Go to Agents in the sidebar. You\'ll see cards for each of the 7 roles. Click any card to expand its configuration.', screenshots: [{ filename: 'agents-page-grid.png', caption: 'Agents page showing the 7 role cards (Orchestrator, Appointment, Sales, Customer Service, Marketing, Assistant, Secretary)', capture: { url: '/agents', authAs: 'tenant' } }] },
          { title: 'Enable the role', body: 'Toggle the "Enabled" switch at the top of the card. Disabled roles are completely inactive — they don\'t affect conversations at all.', screenshots: [{ filename: 'agents-enable-toggle.png', caption: 'Agent role card expanded with the \'Enabled\' toggle in the top-right', capture: { url: '/agents', authAs: 'tenant' } }] },
          { title: 'Bind a prompt', body: 'Use the "Role Prompt" dropdown to select a published prompt with matching scope. If you haven\'t created a role-specific prompt yet, the agent uses only the master prompt — which is fine for most use cases.', screenshots: [{ filename: 'agents-prompt-dropdown.png', caption: 'Agent role configuration showing the \'Role Prompt\' dropdown with available published prompts listed', capture: { url: '/agents', authAs: 'tenant' } }] },
          ...
          { title: 'Save', body: 'Click Save. Changes take effect on the next new conversation — existing live sessions are not interrupted.', screenshots: [{ filename: 'agents-save-button.png', caption: 'Bottom of agent role card showing the Save button', capture: { url: '/agents', authAs: 'tenant' } }] },
        ],
    ...
    articles: [
      {
        ...
        steps: [
          ...
          { title: 'Select a channel tab', body: 'At the top of Agent Studio you\'ll see three tabs: Widget, Inbound, and Outbound. Select the channel you want to test. Each channel may behave differently based on its channel overlay prompt.', screenshots: [{ filename: 'studio-channel-tabs.png', caption: 'Agent Studio top with the three channel tabs (Widget / Inbound / Outbound), Widget tab active', capture: { url: '/agent-studio', authAs: 'tenant' } }] },
          { title: 'Widget tab — voice and avatar', body: 'The Widget tab lets you pick an avatar (the face shown in the chat widget) and a voice. Select a female or male avatar from the grid, then choose a voice from the dropdown. These settings are specific to the widget.', screenshots: [{ filename: 'studio-avatar-grid.png', caption: 'Widget tab showing the female/male avatar selection grid', capture: { url: '/agent-studio', authAs: 'tenant' } }, { filename: 'studio-voice-dropdown.png', caption: 'Widget tab voice picker dropdown open, listing the 7 voices (Zephyr, Despina, Aoede, Charon, Fenrir, Puck, Sulafat)', capture: { url: '/agent-studio', authAs: 'tenant' } }] },
    ...
    articles: [
      {
        ...
        steps: [
          { title: 'Enable the Widget channel', body: 'Go to Channels in the sidebar. Click the Widget card. Toggle "Enable Widget" to on. The widget is now active — but not yet on your website.', screenshots: [{ filename: 'channels-widget-enable-toggle.png', caption: 'Channels page Widget card with the \'Enable Widget\' toggle', capture: { url: '/channels', authAs: 'tenant' } }] },
          { title: 'Widget position', body: 'Choose whether the widget button appears in the bottom-right or bottom-left corner of your website. Most websites use bottom-right.', screenshots: [{ filename: 'channels-widget-position-dropdown.png', caption: 'Widget configuration showing the position dropdown (Bottom-Right / Bottom-Left)', capture: { url: '/channels', authAs: 'tenant' } }] },
          { title: 'Widget color', body: 'Set the widget button color to match your brand. Use a hex code or pick from the presets. This only affects the trigger button — the chat panel uses the MyOrbisVoice theme.', screenshots: [{ filename: 'channels-widget-color-picker.png', caption: 'Widget configuration showing the color picker / hex input field', capture: { url: '/channels', authAs: 'tenant' } }] },
          ...
          { title: 'Copy the embed snippet', body: 'At the bottom of the Widget configuration, click "Copy Embed Code". You\'ll get a short <script> tag.', screenshots: [{ filename: 'channels-widget-copy-embed.png', caption: 'Widget configuration with the \'Copy Embed Code\' button at the bottom and the script tag preview', capture: { url: '/channels', authAs: 'tenant' } }] },
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'Enable Inbound channel', body: 'Go to Channels → Inbound. Toggle "Enable Inbound Receptionist" to on.', screenshots: [{ filename: 'channels-inbound-enable.png', caption: 'Channels page Inbound Receptionist card with the \'Enable\' toggle and webhook URL display', capture: { url: '/channels', authAs: 'tenant' } }] },
          { title: 'Set the inbound greeting', body: 'This is the first thing callers hear. Keep it short: "[Business name], [agent name] speaking, how can I help?" The agent then takes over the conversation.', screenshots: [{ filename: 'channels-inbound-greeting-field.png', caption: 'Inbound configuration with the greeting text field showing the greeting input', capture: { url: '/channels', authAs: 'tenant' } }] },
          { title: 'After-hours behavior', body: 'Set what happens when a caller calls outside your business hours (set in Business DNA). Options: (1) The agent still answers but informs the caller of hours and takes a message. (2) The caller hears a voicemail-style recording. (3) Forward to another number.', screenshots: [{ filename: 'channels-inbound-after-hours.png', caption: 'Inbound configuration showing the after-hours behavior options (agent answers / voicemail / forward)', capture: { url: '/channels', authAs: 'tenant' } }] },
          { title: 'Call forwarding / transfer', body: 'Set the phone number to forward to when the agent escalates a call to a human. This is the number a human staff member will receive on their phone when the agent decides to transfer.', screenshots: [{ filename: 'channels-inbound-forwarding-number.png', caption: 'Inbound configuration showing the forwarding-number field with E.164 format hint', capture: { url: '/channels', authAs: 'tenant' } }] },
          { title: 'Configure the Twilio webhook', body: 'In Twilio, set the voice webhook URL for your phone number to: https://api.myorbisvoice.com/api/webhooks/twilio/voice. This tells Twilio to send incoming calls to MyOrbisVoice.' },
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'Enable Outbound channel', body: 'Go to Channels → Outbound. Toggle "Enable Outbound Caller" to on.', screenshots: [{ filename: 'channels-outbound-enable.png', caption: 'Channels page Outbound Caller card with the \'Enable\' toggle', capture: { url: '/channels', authAs: 'tenant' } }] },
          { title: 'Set caller ID', body: 'Choose which phone number appears on the recipient\'s phone when the agent calls them. This must be a Twilio number in your account.', screenshots: [{ filename: 'channels-outbound-caller-id.png', caption: 'Outbound configuration showing the caller-ID dropdown listing available Twilio numbers', capture: { url: '/channels', authAs: 'tenant' } }] },
          { title: 'Retry policy', body: 'Set how many times the agent should retry a number that doesn\'t answer or goes to voicemail. Recommended: 2 retries, 4 hours apart.', screenshots: [{ filename: 'channels-outbound-retry-policy.png', caption: 'Outbound configuration showing retry count and retry-interval fields', capture: { url: '/channels', authAs: 'tenant' } }] },
...
{ title: 'Start the connection', body: 'Go to Integrations in the sidebar. Click "Connect Google". You\'ll be redirected to Google\'s login page.', screenshots: [{ filename: 'integrations-google-connect-button.png', caption: 'Integrations page Google card showing the \'Connect Google\' button (NOT_CONNECTED state)' }] },
          { title: 'Confirm the connection', body: 'You\'ll be redirected back to MyOrbisVoice. The Google card will show "Connected" with the email address of the connected account and the number of calendars found.', screenshots: [{ filename: 'integrations-google-connected-state.png', caption: 'Integrations page Google card now in the CONNECTED state, showing email + calendar count + last verified timestamp' }] },
...
{ title: 'Find your credentials', body: 'In the Twilio Console, go to the main dashboard. You\'ll see your Account SID and Auth Token. Copy both.' },
          { title: 'Enter credentials in MyOrbisVoice', body: 'Go to Integrations → Twilio. Enter your Account SID and Auth Token in the fields provided. Click Save. The fields are write-only — once saved, the token is never displayed again.', screenshots: [{ filename: 'integrations-twilio-credentials-form.png', caption: 'MyOrbisVoice Integrations page Twilio card with Account SID / Auth Token / Phone Number input fields' }] },
          { title: 'Add the number in MyOrbisVoice', body: 'Go to Phone Numbers in the MyOrbisVoice sidebar. Click "Add Number". Enter the Twilio number in E.164 format (e.g. +14155551234). Assign it to the Inbound channel.', screenshots: [{ filename: 'phone-numbers-add-modal.png', caption: 'Phone Numbers page with the \'+ Add number\' modal open, showing the E.164 input field', capture: { url: '/phone-numbers', authAs: 'tenant' } }] },
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'Quick path for inbound only', body: 'If you only need inbound calls (no outbound, no SMS), you can skip every approval. Buy a Twilio number, set its Voice webhook to https://api.myorbisvoice.com/api/webhooks/twilio/voice, connect to MyOrbisVoice. Done in 5 minutes. The rest of these articles only matter for outbound voice or SMS.' },
        ],
        tips: [
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'If Twilio asks about the SaaS platform', body: 'They sometimes ask "what software is involved?" Use this answer — it positions YOU as the sender of record (correct), not MyOrbisVoice.', template: { label: 'SaaS platform disclosure', content: 'MyOrbisVoice provides the software used by [CLIENT BUSINESS NAME] to manage calls, SMS messages, appointment scheduling, and customer follow-up. [CLIENT BUSINESS NAME] is the sender of record and controls its own customer communications, phone numbers, opt-in records, and business messaging use case.' } },
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'API Keys (for connecting to MyOrbisVoice)', body: 'After approvals are done, create an API Key to connect Twilio to MyOrbisVoice (better than using your main Auth Token).', link: { label: 'Open API Keys →', href: 'https://console.twilio.com/us1/account/keys-credentials/api-keys' } },
        ],
        tips: [
      ...
      },
      {
        ...
        tips: [
          'Print this checklist (or take screenshots) and check items off as you complete them. The checklist UI for tracking this in MyOrbisVoice is on our roadmap but not built yet.',
        ],
      },
    ],
  },
    ...
    articles: [
      {
        ...
        steps: [
          ...
          { title: 'Viewing appointments in MyOrbisVoice', body: 'Go to Appointments in the sidebar. You\'ll see all appointments booked through the agent with status (Scheduled, Completed, Cancelled). You can also cancel appointments manually from here.' },
        ],
    ...
    articles: [
      {
        ...
        steps: [
          ...
          { title: 'Finding a conversation', body: 'Go to Conversations. Use the search bar to find by contact name or phone number. Use the filters to narrow by channel, status, or date range. Records are sorted newest first by default.', screenshots: [{ filename: 'conversations-list-with-filters.png', caption: 'Conversations page showing the list, search bar, and channel/status/date filter controls', capture: { url: '/conversations', authAs: 'tenant' } }] },
    ...
    articles: [
      ...
      },
      {
        ...
        steps: [
          ...
          { title: 'Usage warnings', body: 'MyOrbisVoice will show a yellow warning when you reach 80% of your monthly limit and a red warning at 95%. These appear in the Usage page and on your dashboard.' },
    ...
    articles: [
      {
        ...
        steps: [
          { title: 'Workspace display name', body: 'The workspace name appears in your dashboard header and in admin views. It does not affect the agent\'s behavior — the agent uses the name from Business DNA.', screenshots: [{ filename: 'settings-workspace-fields.png', caption: 'Settings page Workspace section with display name, timezone, and notification email fields', capture: { url: '/settings', authAs: 'tenant' } }] },
          ...
          { title: 'Notification email', body: 'The email address where MyOrbisVoice sends account notifications: usage warnings, billing receipts, and integration alerts. This can be different from your login email.' },
