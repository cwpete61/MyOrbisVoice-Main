    ...
    <div className="max-w-2xl">
            ...
            <div className="flex-1">
              ...
              {step.link && (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 mt-2 px-3 py-1.5 rounded-md text-xs font-semibold"
      ...
      {article.tips && article.tips.length > 0 && (
        <div className="rounded-xl p-4 mb-4" style={{ background: 'oklch(55% 0.11 193 / 0.08)', border: '1px solid oklch(55% 0.11 193 / 0.2)' }}>
          <p className="text-xs font-bold mb-2" style={{ color: 'oklch(45% 0.11 193)' }}>TIPS</p>
      ...
      {article.warnings && article.warnings.length > 0 && (
        <div className="rounded-xl p-4" style={{ background: 'oklch(60% 0.18 30 / 0.08)', border: '1px solid oklch(60% 0.18 30 / 0.25)' }}>
          <p className="text-xs font-bold mb-2" style={{ color: 'oklch(45% 0.16 30)' }}>IMPORTANT</p>
          <ul className="space-y-1.5">
...
export default function AdminHelpPage() {
  const [activeArticleId, setActiveArticleId] = useState<string>(
    ADMIN_HELP_CONTENT[0]!.articles[0]!.id
  )
  const [search, setSearch] = useState('')
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>(
    Object.fromEntries(ADMIN_HELP_CONTENT.map(s => [s.id, true]))
  )
  ...
  useEffect(() => {
    function applyHash() {
      ...
      if (!hash) return
      for (const section of ADMIN_HELP_CONTENT) {
  ...
  const filtered = useMemo(() => {
    if (!search.trim()) return ADMIN_HELP_CONTENT
    const q = search.toLowerCase()
    return ADMIN_HELP_CONTENT.map(section => ({
      ...section,
  ...
  const activeArticle = useMemo(() => {
    ...
    return ADMIN_HELP_CONTENT[0]!.articles[0]!
  }, [activeArticleId])
    ...
    <div className="flex gap-0" style={{ minHeight: 'calc(100vh - 4rem)', margin: '-2rem' }}>
      <aside className="w-64 flex-shrink-0 flex flex-col" style={{ background: 'var(--surface-raised)', borderRight: '1px solid var(--border-subtle)' }}>
        <div className="px-4 py-4" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
          <p className="text-sm font-bold mb-1" style={{ color: 'var(--text-primary)' }}>Admin Help</p>
          <p className="text-xs mb-3" style={{ color: 'var(--text-tertiary)' }}>Operator + support staff guide</p>
        ...
        <div className="px-4 py-3 border-t" style={{ borderColor: 'var(--border-subtle)' }}>
          <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>
            More articles coming — see CLAUDE.md backlog #13 for the full inventory.
          </p>
        </div>
      </aside>
