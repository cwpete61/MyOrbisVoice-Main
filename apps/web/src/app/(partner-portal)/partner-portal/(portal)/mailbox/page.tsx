...
export default function PartnerInboxPage() {
  ...
  useEffect(() => {
    apiFetch<ListResponse>('/api/partner/mailbox/inbox?limit=50')
  ...
  async function loadMore() {
    ...
    const d = await apiFetch<ListResponse>(`/api/partner/mailbox/inbox?cursor=${nextCursor}&limit=50`)
    ...
    <div className="max-w-3xl">
      ...
      {!loading && !error && items.length === 0 && (
        <div className="rounded-xl p-8 text-center" style={{ background: 'var(--surface-raised)', border: '1px solid var(--border-subtle)' }}>
          <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{t('partnerMailbox.emptyInbox')}</p>
          <p className="text-xs mt-2" style={{ color: 'var(--text-tertiary)' }}>{t('partnerMailbox.emptyInboxHelp')}</p>
        </div>
      )}
