...
/**
 * Webinar Marketing — list detail. Shows pipeline status, counts per stage,
 * action buttons (Start discovery / Archive / Export CSV), and a link to
...
export default function WebinarListDetailPage() {
  const params = useParams<{ id: string }>()
  const listId = params.id
  const { data: list, loading, error, reload } = useApi<LeadListDetail>(
    `/api/partner/webinar-marketing/lists/${listId}`,
  )
  ...
  if (error || !list) {
    return <div style={{ padding: 32, color: '#ef4444' }}>Error: {String(error)}</div>
  }
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
      <Link
        href="/partner-portal/webinar-marketing"
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 20 }}>
        <button
          type="button"
          disabled={busy || !canStartDiscovery}
          onClick={() => action('POST', `/api/partner/webinar-marketing/lists/${list.id}/discover`, 'Discovery queued')}
          style={actionBtn(true, busy || !canStartDiscovery)}
        >
          {list.status === 'DRAFT' ? '▶ Start discovery' : '↻ Re-enqueue queries'}
        </button>
        <Link
          href={`/partner-portal/webinar-marketing/${list.id}/queue`}
          style={actionLink(false)}
        >
          Review queue →
        </Link>
        <a
          href={canExport ? `/api/partner/webinar-marketing/lists/${list.id}/export` : '#'}
        ...
        <button
          ...
          onClick={() => action('DELETE', `/api/partner/webinar-marketing/lists/${list.id}`, 'List archived')}
