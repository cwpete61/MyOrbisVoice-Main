...
function NewWebinarForm({ onCreated }: { onCreated: () => void }) {
  ...
  return (
    <form onSubmit={submit} style={{ background: 'var(--surface-raised)', border: '1px solid var(--border-subtle)', borderRadius: 12, padding: 18, marginTop: 12, display: 'grid', gap: 12 }}>
      ...
      <div><label style={label}>Video <span style={{ fontWeight: 400, color: 'var(--text-tertiary)' }}>— YouTube/Vimeo link or embed code; needed to publish</span></label><input style={input} value={videoUrl} onChange={e => setVideoUrl(e.target.value)} maxLength={1200} placeholder="https://youtu.be/dQw4w9WgXcQ" /></div>
