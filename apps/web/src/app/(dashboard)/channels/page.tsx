...
function CopyButton({ text, t }: { text: string; t: TFn }) {
  ...
  return (
    <button type="button"
      ...
      style={{ background: 'var(--surface-overlay)', color: 'var(--text-secondary)', border: '1px solid var(--border-subtle)' }}>
      {copied ? t('tenantChannels.widgetEmbed.copied') : t('tenantChannels.widgetEmbed.copy')}
    </button>
  )
}

// Widget embed snippet — shown on the WIDGET channel detail so tenants can copy
// the install code without hunting for the (unlinked) /widget-test page. The
// publicKey is auto-generated by the API the first time the widget is enabled +
// saved, so it only renders once a key exists.
function WidgetEmbedPanel({ publicKey, t }: { publicKey: string | null | undefined; t: TFn }) {
  ...
  return (
    <div className="pt-4 space-y-3" style={{ borderTop: '1px solid var(--border-subtle)' }}>
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{t('tenantChannels.widgetEmbed.title')}</h3>
          <p className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>{t('tenantChannels.widgetEmbed.subtitle')}</p>
        </div>
        {publicKey && <CopyButton text={embedCode} t={t} />}
      </div>
      {publicKey ? (
        ...
        <p className="text-xs italic" style={{ color: 'var(--text-tertiary)' }}>{t('tenantChannels.widgetEmbed.disabledHint')}</p>
      )}
      <Link href="/widget-test" className="inline-block text-xs font-semibold underline" style={{ color: 'oklch(55% 0.11 193)' }}>
        {t('tenantChannels.widgetEmbed.testLink')}
      </Link>
    </div>
  )
}
      ...
      {selected && (
        <>
          ...
          {selected.channelType === 'INBOUND' && (
            <>
              {/* Scheduling mode — Booking vs Callback-only (field-service) */}
              <div className="pt-4 space-y-4" style={{ borderTop: '1px solid var(--border-subtle)' }}>
                <h3 className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>{t('tenantChannels.scheduling.title')}</h3>
                <div>
                  <label className={lbl}>{t('tenantChannels.scheduling.modeLabel')}</label>
                  <select value={cfg.scheduling?.mode ?? 'BOOKING'}
                ...
                {cfg.scheduling?.mode === 'CALLBACK' && (
                  <>
                    <div>
                      <label className={lbl}>{t('tenantChannels.scheduling.slaLabel')}</label>
                      <select value={cfg.scheduling?.callbackSla ?? 'SAME_DAY'}
                        onChange={(e) => setConfig('scheduling', { ...(cfg.scheduling ?? {}), callbackSla: e.target.value })}
                        className={inp}>
                        <option value="ONE_HOUR">{t('tenantChannels.scheduling.slaOneHour')}</option>
                        <option value="SAME_DAY">{t('tenantChannels.scheduling.slaSameDay')}</option>
                        <option value="NEXT_BUSINESS_DAY">{t('tenantChannels.scheduling.slaNextDay')}</option>
                      </select>
                    </div>
                    <div>
                      <label className={lbl}>{t('tenantChannels.scheduling.whoLabel')}</label>
                      <input value={cfg.scheduling?.callbackWho ?? ''}
                        onChange={(e) => setConfig('scheduling', { ...(cfg.scheduling ?? {}), callbackWho: e.target.value || undefined })}
                        className={inp} placeholder={t('tenantChannels.scheduling.whoPlaceholder')} />
                    </div>
                    <div>
                      <label className={lbl}>{t('tenantChannels.scheduling.ownerPhoneLabel')}</label>
                      <input value={cfg.scheduling?.ownerNotify?.phone ?? ''}
                        onChange={(e) => setConfig('scheduling', { ...(cfg.scheduling ?? {}), ownerNotify: { ...(cfg.scheduling?.ownerNotify ?? {}), sms: true, phone: e.target.value || undefined } })}
                        className={inp} placeholder="+1…" />
                      <p className="mt-1 text-xs" style={{ color: 'var(--text-tertiary)' }}>{t('tenantChannels.scheduling.ownerPhoneHelp')}</p>
                    </div>
                    <label className="flex items-center gap-2 text-sm" style={{ color: 'var(--text-secondary)' }}>
                      <input type="checkbox" checked={cfg.scheduling?.callerTextBack !== false}
                        onChange={(e) => setConfig('scheduling', { ...(cfg.scheduling ?? {}), callerTextBack: e.target.checked })} />
                      {t('tenantChannels.scheduling.textBackLabel')}
                    </label>
                  </>
                )}
              </div>
          ...
          {selected.channelType === 'WIDGET' && (
            <WidgetEmbedPanel publicKey={selected.publicKey} t={t} />
          )}
