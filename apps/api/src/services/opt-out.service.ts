...
export type OptOutChannel = 'SMS' | 'VOICE' | 'EMAIL'
...
export async function processOptOut(
  ...
  await Promise.all([
    ...
    prisma.optOutLog.create({ data: { tenantId, contactId, channel, source, optedOut: true } }),
  ])
}
...
export async function processOptIn(
  ...
  await Promise.all([
    prisma.contact.update({ where: { id: contactId }, data }),
    prisma.optOutLog.create({ data: { tenantId, contactId, channel, source, optedOut: false } }),
  ])
}
