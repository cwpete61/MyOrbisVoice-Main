// MoreMito × Orby — homepage explainer (~30s, 16:9), built from the pitch-page
// content. Frame-driven motion only (interpolate/spring) per Remotion rules —
// no CSS animations. Parameterized by lang ('en' | 'es') for the bilingual pair.
...
export const MOREMITO_FRAMES = 900 // 30s @ 30fps
...
const T = {
  en: {
    eyebrow: 'AI voice assistant · for people & pets',
  ...
  es: {
    eyebrow: 'Asistente de voz con IA · personas y mascotas',
...
// MoreMito leaf mark (green orb) + wordmark
...
// ---------- Scene 1: HOOK ----------
const Hook: React.FC<{ t: (typeof T)['en'] }> = ({ t }) => {
  const frame = useCurrentFrame()
  const o = fade(frame, 0, 150)
  return (
...
// ---------- Scene 2: WHAT (chips) ----------
const What: React.FC<{ t: (typeof T)['en'] }> = ({ t }) => {
  const frame = useCurrentFrame()
...
// ---------- Scene 3: STATS ----------
const Stats: React.FC<{ t: (typeof T)['en'] }> = ({ t }) => {
  const frame = useCurrentFrame()
  const o = fade(frame, 0, 150)
  return (
    <AbsoluteFill style={{ opacity: o, justifyContent: 'center', alignItems: 'center', padding: 100 }}>
...
// ---------- Scene 4: FEATURES ----------
const Feats: React.FC<{ t: (typeof T)['en'] }> = ({ t }) => {
  const frame = useCurrentFrame()
  const o = fade(frame, 0, 150)
  return (
    <AbsoluteFill style={{ opacity: o, justifyContent: 'center', alignItems: 'center', padding: 120 }}>
...
// ---------- Scene 5: CTA ----------
const Cta: React.FC<{ t: (typeof T)['en'] }> = ({ t }) => {
  const frame = useCurrentFrame()
  const o = fade(frame, 0, 200, 18)
  return (
    <AbsoluteFill style={{ opacity: o, justifyContent: 'center', alignItems: 'center', padding: 120, textAlign: 'center' }}>
...
export const MoreMitoExplainer16x9: React.FC<{ lang?: Lang }> = ({ lang = 'en' }) => {
  const t = T[lang]
  return (
    <AbsoluteFill>
      <Backdrop />
      <Sequence durationInFrames={180}><Hook t={t} /></Sequence>
      <Sequence from={180} durationInFrames={240}><What t={t} /></Sequence>
      <Sequence from={420} durationInFrames={180}><Stats t={t} /></Sequence>
      <Sequence from={600} durationInFrames={180}><Feats t={t} /></Sequence>
      <Sequence from={780} durationInFrames={120}><Cta t={t} /></Sequence>
    </AbsoluteFill>
  )
}
