...
- When Orby reads spelled input back (emails, names), ALWAYS use the phonetic alphabet ("F as in Frank"), NEVER a run of bare single letters. The TTS collapses letter runs into words — spelling "c-r-a-w-f-o-r-d" makes it pronounce "f-o-r" as the number "four" (a real caller spent a whole call fighting it). Phonetic words are unambiguous and also fix P/T/B/D and M/N phone confusions. Prompt rules live in prompt-resolver.ts + tools.ts (email read-back).
- Call-recording mux places chunks by wall-clock arrival time; Gemini streams Orby's audio FASTER than real time (bursts), so lay each stream's chunks with a per-stream cursor (consecutive, never overlapping the same stream) — otherwise her audio piles up at one timeline spot and the RECORDING plays jumbled even though the live call is fine (muxToWav in inbound.ts).
